...
<#macro "container-panel">
    <div<#if .node["@id"]?has_content> id="${.node["@id"]}"</#if>
        ...
        <div<#if .node["@id"]?has_content> id="${.node["@id"]}-header"</#if>
                  data-dojo-type="dijit.layout.ContentPane" region="top"
                  class="panel-header ui-helper-clearfix"><#recurse .node["panel-header"][0]>
        </div></#if>
        <#if .node["panel-left"]?has_content>
        <#-- LATER <xs:attribute name="draggable" default="false" type="boolean"/> -->
        <div<#if .node["@id"]?has_content> id="${.node["@id"]}-left"</#if> 
                  data-dojo-type="dijit.layout.ContentPane" region="left"
                  class="panel-left"><#recurse .node["panel-left"][0]>
        </div></#if>
        <#if .node["panel-right"]?has_content>
        <div<#if .node["@id"]?has_content> id="${.node["@id"]}-right"</#if> 
                  data-dojo-type="dijit.layout.ContentPane" region="right"
                  class="panel-right"><#recurse .node["panel-right"][0]>
        </div></#if>
        <div<#if .node["@id"]?has_content> id="${.node["@id"]}-center"</#if> 
                  data-dojo-type="dijit.layout.ContentPane" region="center"
                  class="panel-center"><#recurse .node["panel-center"][0]>
        </div>
        <#if .node["panel-footer"]?has_content>
        <div class="ui-helper-clearfix"></div>
        <div<#if .node["@id"]?has_content> id="${.node["@id"]}-footer"</#if> 
                  data-dojo-type="dijit.layout.ContentPane" region="bottom"
                  class="panel-footer"><#recurse .node["panel-footer"][0]>
        </div></#if>
    </div>
</#macro>
