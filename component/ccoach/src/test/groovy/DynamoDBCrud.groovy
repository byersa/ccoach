...
class DynamoDBCrud extends Specification {
    ...
    def "create and find Example TEST1"() {
        when:
        ec.entity.makeValue("Person").setAll([partyId:"TEST1", lastName:"Name", firstName:"Test"]).createOrUpdate()

        then:
        EntityValue example = ec.entity.makeFind("Person").condition("partyId", "TEST1").one()
